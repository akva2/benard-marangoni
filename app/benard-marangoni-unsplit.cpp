...
int main(int argc, char** argv)
{
  ...
  if( n == 0 ) {
    velocity3D.m_nu = velocity3DN.m_nu = Real(1)/(Pr*Dt);
...
#pragma omp parallel for schedule(static)
    ...
    pnp1 *= -1;
    cout << "iterations pressure " << utilities::CGSolver::solve(pnp1,pressure3D,pnp1.getDotter(),1.e-6) << endl;
    ...
    if( secondorder ) {
      velocity3D.m_nu = velocity3DN.m_nu = Real(3)/(2*Dt*Pr);
