...
#ifndef LEGENDRE_LEGENDRE_H_
#define LEGENDRE_LEGENDRE_H_
...
namespace legendreLegendre {
  ...
  mnl::basics::Matrix makePeriodic(const mnl::basics::Matrix& input, bool both);
  mnl::basics::Matrix unperiodize(const mnl::basics::Matrix& u);
  ...
  void periodicCopy(mnl::basics::Matrix& result, const mnl::basics::Matrix& phi);
  ...
  void diffy(mnl::basics::Matrix& result, const mnl::basics::Matrix& D, const mnl::basics::Matrix& input, const Real scaleA=mnlRealOne, const Real scaleB=mnlRealZero);
  ...
  inline void mass(mnl::basics::Matrix& result, const mnl::basics::Vector& weightX,
      const mnl::basics::Vector& weightY, Real scaleX=M_PI, Real scaleY=1)
  {
    massX(result,weightX,scaleX);
    massY(result,weightY,scaleY);
  }
  inline void mass(mnl::basics::Field2D& result, const mnl::basics::Vector& weightX,
      const mnl::basics::Vector& weightY, Real scaleX=M_PI, Real scaleY=1)
  {
    mass(result.X(),weightX,weightY,scaleX,scaleY);
    mass(result.Y(),weightX,weightY,scaleX,scaleY);
  }
